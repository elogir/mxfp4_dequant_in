...
pub const TensorInfo = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, name: []const u8, tensor_obj: json.ObjectMap) !TensorInfo {
    ...
    pub fn size(self: TensorInfo) u64 {
        return self.data_offsets[1] - self.data_offsets[0];
    }
};

pub const MxfpPair = struct {
    blocks: ?*TensorInfo = null,
    scales: ?*TensorInfo = null,

    pub fn isComplete(self: *const MxfpPair) bool {
...
pub const DequantedTensor = struct {
    name: []const u8,
    dtype: []const u8,
    shape: [4]u32,
    shape_len: usize,
...
fn loadTensorsFromHeader(allocator: std.mem.Allocator, parsed_header: *const json.Value) ![]TensorInfo {
    ...
    while (iter.next()) |entry| {
        ...
        if (std.mem.eql(u8, tensor_name, "__metadata__")) continue; // pour le parsing on s'en fou on le copiera plus tard
